...
void OLED_WR_Byte(uint8_t dat, uint8_t cmd)
{
	//uint8_t i;
	if (cmd)
		OLED_DC_Set(); //DC引脚  1：命令  0： 数据
	else
		OLED_DC_Clr(); //DC引脚  1：命令  0： 数据
	//OLED_CS_Clr();
	HAL_SPI_Transmit(&hspi1, &dat, sizeof(dat), 100);
	// OLED_CS_Clr();
	// for (i = 0; i < 8; i++)
	// {
	// 	OLED_SCLK_Clr(); //SCLK =0  模拟SPI时钟

	// 	//移位写入数据
	// 	if (dat & 0x80) //数据高位为1
	// 		OLED_SDIN_Set();
	// 	else
	// 		OLED_SDIN_Clr(); //数据高位为0
	// 	OLED_SCLK_Set();	 //SCLK =1 模拟SPI时钟
	// 	dat <<= 1;
	// }
	//OLED_CS_Set();
	OLED_DC_Set();
...
void OLED_Init(void)
{
	...
	// GPIO_Init(GPIOB, &GPIO_InitStructure);			  //初始化GPIOB 1， 12， 13， 14， 15
